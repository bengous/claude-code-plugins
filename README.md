...
| [context-management](context-management/) | 2.0.1 | Lifecycle of Claude Code instruction files: audit CLAUDE.md/AGENTS.md/.claude/rules instruction budget, capture session learnings, and resync docs with codebase evolution |
| [research-tools](research-tools/) | 1.2.3 | Documentation research through optimal MCP sources and claim verification: research agents, source routing, web and codebase fact-checking |
...
| [git-worktree](git-worktree/) | 1.2.2 | Git worktree helper with stack support for multi-agent orchestration |
| [agents-bridge](agents-bridge/) | 1.11.1 | Bridge to the OpenAI Codex CLI for cross-model collaboration |
| [software-craft](software-craft/) | 3.0.1 | Opinionated design skills for software excellence: CLI design, system architecture, and more. Skills that push Claude toward intentional, high-quality output instead of generic patterns. |
| [understanding](understanding/) | 1.1.2 | Understand a topic, a bug, or a Claude Code workflow: minimal explanations, root-cause diagnosis, execution traces |
| [design-studio](design-studio/) | 1.1.2 | Generate 5 unique website redesigns using Theo's pattern: one agent, sequential creation, natural differentiation |
| [plugin-cache-sync](plugin-cache-sync/) | 2.2.2 | Sync Claude Code plugin cache from local sources |
| [git-sweep](git-sweep/) | 3.2.1 | Interactive git branch and worktree cleanup: proves a branch is contained before proposing it, frees finished worktrees, and reports what it kept and why |
| [goalify](goalify/) | 1.0.1 | Convert rough intent into the smallest useful goal payload to hand to a fresh Claude Code agent (new session, subagent, or /loop). Draft-first by default, or interactive question-first. |
...
| [vellum](vellum/) | 0.13.0 | Plan at the frontiers and review the plan in the browser: decisions and interfaces before mechanics, vertical slices each closed by a check. /vellum:start opens a planning mode of its own: Claude writes the plan and its mockups and grills the reviewer on the open choices; the reviewer comments, edits or approves in a page, and the answer reaches Claude as a prompt. |
