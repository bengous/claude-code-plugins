...
Four jobs, following the table of [Which suite runs on which trigger](#which-suite-runs-on-which-trigger). `validate` is the ladder above, and the one `scripts/check-lint-config.ts` reads for parity: it runs on every event, any label included, with no `if:`. A job its `if:` skips reports success to a required check (GitHub's [Troubleshooting required status checks](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/collaborating-on-repositories-with-code-quality-features/troubleshooting-required-status-checks)), and `main` requires `validate` ([GitHub branch protections](protections.md)): a skipped `validate` beside a red one could hide it, where a real run per label costs under a minute. `e2e-window` runs vellum's browser suite on request only, one job per window: a matrix over the projects of `WINDOWS` in `vellum/e2e/playwright.config.ts`, with `fail-fast: false`, so a red window cancels none of the others. Each job, `e2e (<window>)`, runs `bun run --cwd vellum e2e -- --project=<window>` after Playwright installs its Chromium with the runner's system libraries (`playwright install --with-deps`, which wants root: locally, `bun run --cwd vellum e2e:install` installs the browser alone). `scripts/e2e-windows.test.ts` holds the matrix equal to the config's projects: a window added to one fails `bun test` until the other has it. `e2e` is the check the e2e gate reads: it needs the windows and runs on the same requests, under `always()`, and fails unless every window's job succeeded, so a red or cancelled window turns it red. No hook runs the suite: it takes seconds per file, and `pre-push` already runs every gate. `unlabel` takes the `e2e` label off the pull request as that run starts, so the label asks for one run and the next request is a new label. The workflow's token reads contents only; `unlabel` alone gets `pull-requests: write`, and a re-run of the same run skips it, the label being gone already.
...
| One-shot `e2e` label on a PR | yes | yes, a job per window; the workflow removes the label |
| Any other label on a PR | yes, never skipped | no |
| `workflow_dispatch`, any ref | yes | yes, a job per window |
