...
6. **Send feedback** writes `.review/vN.feedback.md` (path, then lines and quote, or for a mockup's element its selector, its role and name, the heading it sits under, its opening tag and its text; then the comment, for each) and submits a prompt: Claude reads the file, revises, and `vN+1` is submitted when its turn ends. With an edit the file says first that the reviewer edited `plan.md` and that those edits stay; an edit alone, with no comment, can be sent.
...
| `$.clock.every` | The heartbeat that keeps the server alive, and the slow retry while the server is lost. |
